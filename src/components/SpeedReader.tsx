...
export default function SpeedReader({
  ...
  const [showFocusGuides, setShowFocusGuides] = useState(() => {
    try {
      const stored = localStorage.getItem('readflow_focus_guides');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('readflow_focus_guides', String(showFocusGuides));
    } catch {
  ...
  const renderReadingStage = () => {
    ...
    if (readingMode === 'teleprompter' || readingMode === 'highlight') {
      const start = Math.max(0, currentIndex - 20);
      const end = Math.min(totalWords, currentIndex + groupSize + 45);
      return (
        <div className="max-w-3xl px-6 text-left leading-relaxed text-base md:text-lg">
          {words.slice(start, end).map((word, offset) => {
            const index = start + offset;
            const active = index >= currentIndex && index < currentIndex + groupSize;
            return (
              <span key={`${word}-${index}`} className={active ? 'bg-[#4A6CFF] text-white font-bold rounded px-1 mx-0.5' : 'opacity-65 mx-0.5'}>
                {word}
              </span>
            );
          })}
  ...
  return (
    <div id="speed-reader-view" className={`fixed inset-0 z-40 flex flex-col overflow-hidden transition-all ${currentTheme.bg} ${currentTheme.text} ${currentTheme.fontClass}`}>
      ...
      <main className="flex-1 min-h-0 flex flex-col items-center justify-center px-4 md:px-6 py-4 relative">
        <div className={`w-full max-w-4xl flex flex-col items-center justify-center min-h-[260px] md:min-h-[320px] relative rounded-2xl border ${currentTheme.border} bg-black/[0.015] dark:bg-white/[0.015]`}>
          {renderReadingStage()}
        </div>
      </main>
      ...
      <footer className={`px-5 md:px-6 py-3 border-t ${currentTheme.border} ${currentTheme.cardBg} flex flex-col gap-3 shrink-0`}>
        <div className="max-w-7xl mx-auto w-full flex flex-col gap-3">
          ...
          {showTools && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-[11px] border-b border-zinc-850/60 pb-3">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] items-center gap-3">
            ...
            <div className="flex items-center justify-start lg:justify-end gap-2 min-w-0 overflow-x-auto">
              <button type="button" onClick={() => setShowTools(prev => !prev)} className={`h-8 px-3 rounded-xl text-[10px] font-bold border transition-all cursor-pointer flex items-center gap-1.5 whitespace-nowrap ${showTools ? 'bg-[#4A6CFF]/15 border-[#4A6CFF]/40 text-[#4A6CFF]' : 'opacity-70 hover:opacity-100'}`} title="Gelişmiş okuma araçlarını göster veya gizle">
                <Settings2 className="w-3.5 h-3.5 shrink-0" /><span>Araçlar</span>
              </button>
